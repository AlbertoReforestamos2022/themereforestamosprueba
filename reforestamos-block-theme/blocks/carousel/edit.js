...
export default function Edit({ attributes, setAttributes, clientId }) {
	const { images, autoplay, interval, showControls, showIndicators } =
		attributes;
				...
				{images.length === 0 ? (
					<MediaUploadCheck>
						<Placeholder
							...
							instructions={__(
								'Select images to create a carousel',
								'reforestamos'
							)}
						>
							<MediaUpload
					...
					<div className="carousel-editor-container">
						...
						<div className="carousel-preview-info">
							<p>
								{__('Preview:', 'reforestamos')} {images.length}{' '}
								{images.length === 1
									? __('image', 'reforestamos')
									: __('images', 'reforestamos')}
								{autoplay &&
									` • ${__('Autoplay', 'reforestamos')}: ${interval}ms`}
							</p>
						</div>
					</div>
				)}
