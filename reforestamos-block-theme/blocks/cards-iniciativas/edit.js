...
export default function Edit({ attributes, setAttributes }) {
	...
	const removeStat = (initiativeIndex, statIndex) => {
		const newInitiatives = [...initiatives];
		newInitiatives[initiativeIndex].stats = newInitiatives[
			initiativeIndex
		].stats.filter((_, i) => i !== statIndex);
		setAttributes({ initiatives: newInitiatives });
	};
			...
			<InspectorControls>
				<PanelBody title={__('Layout Settings', 'reforestamos')}>
					<SelectControl
						...
						options={[
							{
								label: __('Grid', 'reforestamos'),
								value: 'grid',
							},
							{
								label: __('List', 'reforestamos'),
								value: 'list',
							},
							{
								label: __('Masonry', 'reforestamos'),
								value: 'masonry',
							},
							{
								label: __('Featured', 'reforestamos'),
								value: 'featured',
							},
						]}
			...
			<div {...blockProps}>
				<div className="reforestamos-cards-iniciativas__editor">
					<div className="reforestamos-cards-iniciativas__header">
						...
						<Button variant="primary" onClick={addInitiative}>
					...
					{initiatives.length === 0 && (
						<div className="reforestamos-cards-iniciativas__empty">
							<p>
								{__(
									'No initiatives added yet. Click "Add Initiative" to get started.',
									'reforestamos'
								)}
							</p>
						</div>
					...
					{initiatives.map((initiative, index) => (
						<div
							key={index}
							className="reforestamos-cards-iniciativas__item"
						>
							<div className="reforestamos-cards-iniciativas__item-header">
								<h4>
									{__('Initiative', 'reforestamos')}{' '}
									{index + 1}
								</h4>
								<Button
									variant="secondary"
							...
							<TextControl
								...
								onChange={(value) =>
									updateInitiative(index, 'title', value)
								}
								placeholder={__(
							...
							<TextControl
								...
								onChange={(value) =>
									updateInitiative(index, 'category', value)
								}
								placeholder={__(
									'e.g., Reforestación, Educación…',
									'reforestamos'
								)}
							/>
							...
							<TextControl
								...
								onChange={(value) =>
									updateInitiative(index, 'link', value)
								}
							...
							<div className="reforestamos-cards-iniciativas__stats">
								...
								{initiative.stats &&
									initiative.stats.map((stat, statIndex) => (
										<div
											key={statIndex}
											className="reforestamos-cards-iniciativas__stat"
										>
											<TextControl
												label={__(
													'Value',
