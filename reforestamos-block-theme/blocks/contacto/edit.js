		...
		<>
			<InspectorControls>
				<PanelBody title={__('Contact Form Settings', 'reforestamos')}>
					<TextControl
						...
						help={__(
							'Unique identifier for future integration with Communication Plugin',
							'reforestamos'
						)}
						value={formId}
				...
				<PanelBody
					title={__('Contact Information', 'reforestamos')}
					initialOpen={false}
				>
					<TextControl
				...
				<PanelBody
					title={__('Map Settings', 'reforestamos')}
					initialOpen={false}
				>
					<ToggleControl
					...
					{showMap && (
						<TextareaControl
							...
							value={mapEmbedUrl}
							onChange={(value) =>
								setAttributes({ mapEmbedUrl: value })
							}
							rows={3}
						/>
					)}
			...
			<div {...blockProps}>
				<div className="reforestamos-contacto__editor-notice">
					...
					<p>
						{__(
							'This block will display a contact form and contact information on the frontend.',
							'reforestamos'
						)}
					</p>
					{formId && (
						<p>
							<em>
								{__('Form ID:', 'reforestamos')} {formId}
							</em>
						</p>
					)}
				</div>
								...
								{showMap && mapEmbedUrl && (
									<div className="reforestamos-contacto__map-preview">
										<p>
											<em>
												{__(
													'Map will be displayed here on frontend',
													'reforestamos'
												)}
											</em>
										</p>
