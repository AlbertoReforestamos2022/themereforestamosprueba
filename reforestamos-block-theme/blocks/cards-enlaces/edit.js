...
import { PanelBody, RangeControl, SelectControl, TextControl, Button, TextareaControl } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }) {
    ...
    const renderCardEditor = (card, index) => {
        ...
        return (
            <div key={index} className="reforestamos-cards-enlaces__card-editor">
                <div className="reforestamos-cards-enlaces__card-header">
                    <h4>{card.title || __('Card', 'reforestamos') + ' ' + (index + 1)}</h4>
                    <div className="reforestamos-cards-enlaces__card-actions">
                        <Button
                            icon={isEditing ? 'arrow-up-alt2' : 'edit'}
                            label={isEditing ? __('Collapse', 'reforestamos') : __('Edit', 'reforestamos')}
                            onClick={() => setEditingCard(isEditing ? null : index)}
                        />
                        <Button
                            icon="trash"
                ...
                {isEditing && (
                    <div className="reforestamos-cards-enlaces__card-fields">
                        <TextControl
                            ...
                            placeholder={__('Enter card title...', 'reforestamos')}
                        />
                        ...
                        <TextareaControl
                            ...
                            placeholder={__('Enter card description...', 'reforestamos')}
                            rows={3}
                        />
                        ...
                        <TextControl
                            ...
                            help={__('Use an emoji or short text as icon', 'reforestamos')}
                        />
                                    ...
                                    <div className="reforestamos-cards-enlaces__image-control">
                                        ...
                                        <div className="reforestamos-cards-enlaces__image-buttons">
                                            <Button onClick={open} variant="secondary">
                                                {card.image?.url ? __('Change Image', 'reforestamos') : __('Select Image', 'reforestamos')}
                                            </Button>
                                            {card.image?.url && (
                                                <Button
                                                    onClick={() => updateCard(index, 'image', { url: '', alt: '', id: null })}
                                                    isDestructive
                                                    variant="secondary"
                                                >
            ...
            <div {...blockProps}>
                <div className="reforestamos-cards-enlaces__editor">
                    <div className="reforestamos-cards-enlaces__editor-header">
                        ...
                        <p className="reforestamos-cards-enlaces__settings-info">
                            {__('Columns:', 'reforestamos')} {columns} | {__('Style:', 'reforestamos')} {cardStyle}
                        </p>
                    </div>
                    ...
                    {cards.length === 0 && (
                        <p className="reforestamos-cards-enlaces__empty">
                            {__('No cards yet. Click "Add Card" to create your first card.', 'reforestamos')}
                        </p>
                    )}
                </div>
                ...
                {cards.length > 0 && (
                    <div className="reforestamos-cards-enlaces__preview">
                        ...
                        <div className={`reforestamos-cards-enlaces__grid reforestamos-cards-enlaces__grid--cols-${columns}`}>
                            {cards.map((card, index) => (
                                <div key={index} className="reforestamos-cards-enlaces__card">
                                    {card.image?.url && (
                                    ...
                                    <div className="reforestamos-cards-enlaces__card-content">
                                        ...
                                        {card.url && (
                                            <div className="reforestamos-cards-enlaces__card-link">
                                                <span>{__('Learn more', 'reforestamos')} →</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
