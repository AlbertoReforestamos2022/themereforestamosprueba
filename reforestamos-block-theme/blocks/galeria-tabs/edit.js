...
export default function Edit({ attributes, setAttributes }) {
	...
	const removeImage = (galleryIndex, imageIndex) => {
		const newGalleries = [...galleries];
		newGalleries[galleryIndex].images = newGalleries[
			galleryIndex
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={__('Configuración de Galería', 'reforestamos')}
				>
					<SelectControl
						...
						options={galleries.map((gallery, index) => ({
							label:
								gallery.title ||
								`${__('Categoría', 'reforestamos')} ${index + 1}`,
							value: index,
						}))}
						onChange={(value) =>
				...
				<div className="galeria-tabs-editor">
					{galleries.length === 0 ? (
						<div className="galeria-tabs-placeholder">
							<p>
								{__(
									'No hay categorías. Añade una categoría para comenzar.',
									'reforestamos'
								)}
							</p>
