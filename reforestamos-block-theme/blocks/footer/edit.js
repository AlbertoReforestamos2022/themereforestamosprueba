...
export default function Edit({ attributes, setAttributes }) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={__('Configuración del Footer', 'reforestamos')}
				>
					<RangeControl
				...
				<PanelBody
					title={__('Colores', 'reforestamos')}
					initialOpen={false}
				>
					<p>
						<strong>{__('Color de Fondo', 'reforestamos')}</strong>
					</p>
					<ColorPicker
					...
					/>
					<p>
						<strong>{__('Color de Texto', 'reforestamos')}</strong>
					</p>
					<ColorPicker
						color={textColor}
						onChangeComplete={(value) =>
				...
				<PanelBody
					title={__('Redes Sociales', 'reforestamos')}
					initialOpen={false}
				>
					{social.map((item, index) => (
						...
						>
							...
							<TextControl
								...
								value={item.icon}
								onChange={(value) =>
									updateSocial(index, 'icon', value)
								}
								help={__(
									'Ej: facebook, twitter, instagram, linkedin',
									'reforestamos'
								)}
							/>
							<Button
					...
					<div className="row">
						{columns
							...
							.map((column, columnIndex) => (
								<div
									key={columnIndex}
									className={`col-md-${12 / columnCount} footer-column`}
								>
									<RichText
										tagName="h4"
										value={column.title}
