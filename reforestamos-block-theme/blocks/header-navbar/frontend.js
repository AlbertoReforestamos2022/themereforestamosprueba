...
function handleLanguageSwitcher(header) {
    ...
    const activeButton = header.querySelector(`.language-btn[data-lang="${storedLang}"]`);
    if (activeButton) {
...
window.reforestamosRenderMenu = function(menuId, menuHtml) {
    const container = document.querySelector(`.navbar-menu-container[data-menu-id="${menuId}"]`);
    if (container) {
