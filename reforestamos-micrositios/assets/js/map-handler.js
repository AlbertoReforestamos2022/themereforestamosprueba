...
(function ($) {
	...
	window.ReforestamosMap = {
		 ...
		 */
		renderArbolesMarkers(map, arboles) {
			const self = this;
		 ...
		 */
		renderOJAMarkers(map, organizaciones) {
			const self = this;
		...
		createOJAPopup(org) {
			...
			if (org.ciudad && org.estado) {
				html +=
					...
					this.escapeHtml(org.ciudad) +
					', ' +
					this.escapeHtml(org.estado) +
					'</p>';
			}
			...
			if (org.contacto && org.contacto.website) {
				html +=
					'<p><a href="' +
					this.escapeHtml(org.contacto.website) +
					'" target="_blank" class="btn btn-sm btn-primary">Visitar sitio web</a></p>';
			}
		 ...
		 * @param {Array} arboles Array of tree data
		 */
		updateArbolesStats(arboles) {
			const totalArboles = arboles.reduce(
				(sum, arbol) => sum + (arbol.cantidad || 0),
				0
			);
		 ...
		 * @param {Array} organizaciones Array of organization data
		 */
		updateOJAStats(organizaciones) {
			const totalOrgs = organizaciones.length;
			const estados = new Set(organizaciones.map((o) => o.estado));
			const totalJovenes = organizaciones.reduce(
				(sum, org) => sum + (org.miembros || 0),
				0
			);
		 ...
		 * @param {Array} arboles Array of tree data
		 */
		populateArbolesFilters(arboles) {
			const ciudades = new Set(
				arboles.map((a) => a.ciudad).filter(Boolean)
			);
			const especies = new Set(
				arboles.map((a) => a.especie).filter(Boolean)
			);
		 ...
		 * @param {Array} organizaciones Array of organization data
		 */
		populateOJAFilters(organizaciones) {
			const estados = new Set(
				organizaciones.map((o) => o.estado).filter(Boolean)
			);
			const tipos = new Set(
				organizaciones.map((o) => o.tipo).filter(Boolean)
			);
		 ...
		 */
		updateFilteredStats(map, visibleCount) {
			const markerGroup = map.arbolesMarkerGroup;
			if (!markerGroup) return;
		...
		setupOJAFilters(map) {
			...
			$('#filter-estado, #filter-tipo, #filter-actividad').on(
				'change',
				function () {
		...
		filterOJADirectory(map) {
			...
			markerGroup.eachLayer(function (marker) {
				if (map.hasLayer(marker)) {
					...
					html +=
						'<p><strong>Ubicación:</strong> ' +
						self.escapeHtml(org.ciudad) +
						', ' +
						self.escapeHtml(org.estado) +
						'</p>';

					if (org.tipo) {
						html +=
							'<p><strong>Tipo:</strong> ' +
							self.escapeHtml(org.tipo) +
							'</p>';
					}
					...
					if (org.descripcion) {
						html +=
							'<p>' + self.escapeHtml(org.descripcion) + '</p>';
					}

					if (org.contacto && org.contacto.website) {
						html +=
							'<a href="' +
							self.escapeHtml(org.contacto.website) +
							'" target="_blank" class="btn btn-sm btn-primary">Visitar sitio web</a> ';
					}

					html +=
						'<button class="btn btn-sm btn-secondary view-on-map" data-lat="' +
						org.ubicacion.lat +
						'" data-lng="' +
						org.ubicacion.lng +
						'">Ver en mapa</button>';
		 ...
		 * @param {Array} organizaciones Array of organization data
		 */
		renderOJADirectory(organizaciones) {
			const self = this;
			const $directory = $('#oja-directory .directory-list');
			$directory.empty();
